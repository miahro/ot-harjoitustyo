...
class TestTopicRepository(unittest.TestCase):
    ...
    def test_add_topics(self):
        size0 = connection.execute("SELECT COUNT(*) FROM Topics;").fetchone()[0]
        topicrepository.update_db([("testiaihe1",), ("testiaihe2",)]) 
        size1 = connection.execute("SELECT COUNT(*) FROM Topics;").fetchone()[0]
    ...
    def test_id_by_topic(self):
        ...
        reference = connection.execute("SELECT id FROM Topics WHERE topic='testiaihe1'").fetchone()[0]

...
class TestTopicServices(unittest.TestCase):
    ...
    def test_get_all_topics(self):
        result = self.topicservices.get_all_topics()
        self.assertEqual(result, ['yhteenlasku', 'vähennyslasku', 'kertolasku', 'jakolasku'])

    def test_set_active_topic(self):
        self.topicservices.set_active_topic("vähennyslasku")
        self.assertEqual(self.topicservices.active_topic, self.topicservices.topicrepository.id_by_topic("vähennyslasku"))

    def test_return_active_topic(self):
        self.topicservices.active_topic=None
        self.assertEqual(self.topicservices.return_active_topic(), "aihetta ei valittu")
        self.topicservices.set_active_topic("yhteenlasku")
        self.assertEqual(self.topicservices.return_active_topic(),"yhteenlasku")
